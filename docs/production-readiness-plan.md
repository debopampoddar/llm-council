...
| Prompt/context integrity | Implemented with explicit limits | JSON provenance envelopes, common authority rules, grounding/trust-boundary review, deterministic adoption detection, one bounded clean synthesis retry, validator-recommendation recovery, internal-metadata rejection, and fail-closed behavior cover explicit context redirection. This is not a general prompt-injection proof; encoded, paraphrased, multilingual, and indirect attacks remain evaluation targets. |
