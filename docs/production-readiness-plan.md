...
| Prompt/context integrity | Implemented with explicit limits | JSON provenance envelopes, common authority rules, grounding/trust-boundary review, a closed explicit-literal output invariant with Unicode normalization, sanitized bounded draft regeneration, identifier-free synthesis recovery, sanitized validator-recommendation recovery, reserved-metadata rejection, and fail-closed behavior cover measurable literal redirection outcomes. Natural-language narration is a quality warning, not a security verdict. This is not a general prompt-injection proof; encoded, paraphrased, multilingual, and non-literal attacks remain evaluation targets. |
...
| Structured output recovery | Partial | Review parsing recovers multiple envelopes, compact criteria, fractional scores, and valid siblings, enforces exact coverage, and makes one sanitized bounded call for malformed output or omitted drafts. Ollama thinking is disabled for structured calls. Validation makes at most one recovery for an exact-ceiling parse failure or a recommendation that violates the explicit-literal invariant; recovery removes the recognized directive and both paid attempts remain visible. Malformed advisor output does not receive a repair call. |
