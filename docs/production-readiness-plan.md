...
| Prompt/context integrity | Implemented with explicit limits | `EVIDENCE` context removes complete recognized instruction-bearing lines before orchestration; explicit `ANALYSIS_SUBJECT` preserves quoted hostile text as non-authoritative data. JSON provenance envelopes, common authority rules, grounding/trust-boundary review, a closed explicit-literal output invariant with Unicode normalization, bounded recovery, closed reserved-output rejection, exact-containment checks across validator control fields, fail-closed behavior, and private-until-approved synthesis cover measurable redirection outcomes. Natural-language narration is a quality warning, not a security verdict. This is not a general prompt-injection proof; encoded, paraphrased, multilingual, and non-literal attacks remain evaluation targets. |
...
| Structured output recovery | Partial | Review parsing recovers multiple envelopes, compact criteria, fractional scores, and valid siblings, enforces exact coverage, and makes one sanitized bounded call for malformed output or omitted drafts. Ollama thinking is disabled for structured calls. Validation makes at most one recovery for an exact-ceiling parse failure or when an authority-bearing field repeats an attacker-requested literal; recovery removes the recognized directive and both paid attempts remain visible. Malformed advisor output does not receive a repair call. |
...
The review stages now make one bounded sanitized request when a response is malformed
or omits required drafts and preserve the original and recovery artifacts, usage,
and events. Extend bounded repair to malformed advisor JSON. Never silently invent
missing scores or confidence.
