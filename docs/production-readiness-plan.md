...
| Prompt/context integrity | Implemented with explicit limits | JSON provenance envelopes, common authority rules, grounding/trust-boundary review, a closed explicit-literal output invariant with Unicode normalization, sanitized bounded draft regeneration, neutral-label synthesis recovery, closed reserved-output rejection, exact-containment checks across validator control fields, and fail-closed behavior cover measurable literal redirection outcomes. Natural-language narration is a quality warning, not a security verdict. This is not a general prompt-injection proof; encoded, paraphrased, multilingual, and non-literal attacks remain evaluation targets. |
