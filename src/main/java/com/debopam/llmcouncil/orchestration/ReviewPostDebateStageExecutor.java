...
public class ReviewPostDebateStageExecutor implements StageExecutor {
            ...
            try {
                ...
                ModelCallResult result = ModelCallDeadline.call(ctx.executionRegistry(registry).clientForModel(modelId),
                        new ModelCallRequest(ctx.session().id(), stage(), model.id(),
                                             ...
                                             true, model.defaultTimeout()), model);
    ...
    private ReviewEvidence.Batch recoverMissing(CouncilContext ctx, ModelProfile model,
        ...
        List<Draft> missingDrafts = ctx.drafts().stream()
                ...
                .toList();
        events.publish(ctx.session().id(), stage().name(),
                "POST_DEBATE_REVIEW_RECOVERY_STARTED", modelId,
        ...
        try {
            ...
            events.publish(ctx.session().id(), stage().name(),
                    "POST_DEBATE_REVIEW_RECOVERY_FAILED", modelId,
